...
export default {
  ...
  methods: {
    selectUser (user) {
      this.$store.dispatch('usuarios/selected', user)
    },
    dropProject (board) {
      console.log(board)
      /*  const p = this.$store.state.usuarios.selected */

      /*  if (p && p.status !== board) {
        request.post('usuarios/update', { id: p.id, status: board },
          response => {
            toast(this, 'success', 'Projeto Atualizado', 'Projeto atualizado com sucesso')
            this.$store.dispatch('usuarios/edit', response.data.projetos)
          },
          error => {
            toast(this, 'danger', 'Usuário não atualizado', error)
